"""
This code is mode to do some calculation of features, scores of different experts. 
It is used to process data from part1 algs.
"""
...
df['Actual_Foldability_MFE'] = np.nan
df['Actual_Foldability_WOMFE'] = np.nan
df['Actual_abs_Foldability_difference'] = np.nan
df['RNAfold_Foldability_MFE'] = np.nan
df['RNAfold_Foldability_WOMFE'] = np.nan
df['RNAfold_abs_Foldability_difference'] = np.nan
df['Our_Foldability_MFE'] = np.nan
df['Our_Foldability_WOMFE'] = np.nan
df['Our_abs_Foldability_difference'] = np.nan
df['Actual_FE'] = np.nan
df['RNAfold_FE'] = np.nan
df['Our_FE'] = np.nan
...
for row in range(0,len(df)):
    ...
    #and RNA_name in txt_name_list:
        ...
        RNAfold_str = df['RNAfold_str'][row]
        ...
        Actual_FE  = FreeEnergyCalculation(new_seq, Actual_str)
        RNAfold_FE  = FreeEnergyCalculation(new_seq, RNAfold_str)
        Our_FE  = FreeEnergyCalculation(new_seq, Our_alg_str)
        ...
        df['Actual_FE'][row] = Actual_FE
        df['RNAfold_FE'][row] = RNAfold_FE
        df['Our_FE'][row] = Our_FE

df.to_csv(os.path.join(path_of_destination, 'results_with_scores.csv'))

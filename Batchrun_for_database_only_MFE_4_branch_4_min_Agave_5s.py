...
import os
import Main_SeqAttRollOut_RNAfold_only_MFE_4_branch_4_min_Agave_5s as Our_alg_Main
import time
#*************************************************************
...
def Put_something_into_txt(title, something):
    ...
    with file("running_time_5s_RNA_MFE_4_branch_4_min_ori_ENTRNA_Agave.txt", "a") as txtfile:
        txtfile.write(title)
        txtfile.write("\n") 
...
def Put_something_into_csv(something):
    #python2 can use file instaed of open
    with file("Result_5s_RNA_MFE_4_branch_4_min_ENTRNA_Agave.csv", "a") as csvfile:
        writer = csv.writer(csvfile)
...
for file_name in os.listdir(path_of_ori_data):
    ...
    Actual_str = Read_Actual_str(path_of_ori_data,file_name)
    RNAfold_str = Read_RNAfold_str(path_of_ori_data,file_name)
    Our_alg_str = Read_Our_alg_str(path_of_ori_data,file_name)

    Actual_foladability = Calculate_Foldability(Ori_Seq,Actual_str)
    RNAfold_foladability = Calculate_Foldability(Ori_Seq,RNAfold_str)
    Our_foldablity = Calculate_Foldability(Ori_Seq,Our_alg_str)

    RNAfold_distance = Calculate_Distance_str(Actual_str,RNAfold_str)
    Our_distance = Calculate_Distance_str(Actual_str,Our_alg_str)
    list_for_the_case=[The_name, Ori_Seq, Actual_str, RNAfold_str, Our_alg_str, Actual_foladability, RNAfold_foladability, Our_foldablity, RNAfold_distance, Our_distance]
    Put_something_into_csv(list_for_the_case)
"""
...
print('scaler, clf',scaler, clf)
print('scaler_ori, clf_ori',scaler_ori, clf_ori)
Ori_Seq = ''
new_seq = Transfer_Ori_Seq_To_Our_Form(Ori_Seq)
new_seq = Transfer_T_into_U(new_seq)
